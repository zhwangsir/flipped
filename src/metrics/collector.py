...
class MetricsCollector:
    ...
    def record_usage(self, *, prompt_tokens: int, completion_tokens: int, calls: int = 1,
                     latency: float | None = None) -> None:
        """直接记账 token 用量(无 LLMResult 的来源,如 OpenHands Worker 的沙盒会话统计)。

        M150 修复：支持传入 latency，修复 metrics 端点 latency/avg_ttft 全 0 的问题。
        """
        with self._lock:
            ...
            if latency is not None:
                self._llm.total_latency += max(0.0, latency)
